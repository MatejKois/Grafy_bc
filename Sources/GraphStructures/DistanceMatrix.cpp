...
namespace Grafy
{
            ...
            : size_(size + 1)
    {
        ...
        for (int y = 0; y < size_; ++y)
        {
            ...
            {
                matrix_[y * size_ + x] = MATRIX_DIST_INF;
            }
            matrix_[y * size_ + y] = 0; // adds 0 for distance between the same node
            matrix_[y * size_] = y; // adds the row-vertex number at the row start for indexing purposes
        }
    }

    DistanceMatrix::DistanceMatrix(int size, int* array)
            : size_(size + 1)
    {
        matrix_ = new int[size_ * size_];
        std::memcpy(matrix_, array, size_ * size_ * sizeof(int));
    }
